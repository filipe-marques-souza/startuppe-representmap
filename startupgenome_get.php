...
$interval_query = mysqli_query($connection, "SELECT sg_lastupdate FROM settings LIMIT 1");
    ...
    try {
      ...
      foreach ($places_arr['response'] as $key => $place) {
        ...
        switch ($place['categories'][0]['parent_category_id']) {
          ...
          case '3': $place['type'] = 'investidor'; break;
          case '4': $place['type'] = 'aceleradora'; break;
          case '5': $place['type'] = 'incubadora'; break;
        ...
        $place_query = mysqli_query($connection, "SELECT id FROM places WHERE sg_organization_id='".$place['organization_id']."' LIMIT 1") or die(mysql_error());

        // organization doesn't exist, add it to the db
        if(mysqli_num_rows($place_query) == 0) {
          mysqli_query($connection, "INSERT INTO places (approved,
                                          title,
                                          type,
                                          lat,
                                          lng,
        ...
        // organization already exists, update it with new info if necessary
          ...
          if($place_info['title'] != $place['name'] || $place_info['type'] != $place['type'] || $place_info['lat'] != $place['latitude'] || $place_info['lng'] != $place['longitude'] || $place_info['address'] != $place['address'] || $place_info['uri'] != $place['url'] || $place_info['description'] != $place['description']) {
            mysqli_query($connection, "UPDATE places SET title='".parseInput($place['name'])."',
      ...
      $deleted = mysqli_query($connection, "DELETE FROM places WHERE sg_organization_id NOT IN ({$org_array})") or die(mysql_error());

      // update settings table with the timestamp for this sync
      mysqli_query($connection, "UPDATE settings SET sg_lastupdate='".time()."'");
