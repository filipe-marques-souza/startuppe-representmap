...
if(isset($_POST['installer_submitted'])) {
	...
	if(!$error) {
		...
		$connection = mysqli_connect($_POST['db_hostname'], $_POST['db_username'], $_POST['db_password'], $_POST['db_name']) or die(mysql_error());

		error_reporting(E_ALL ^ E_DEPRECATED);

		if(!mysqli_num_rows( mysqli_query($connection, "SHOW TABLES LIKE 'events'"))) {
			if(!mysqli_query($connection, "CREATE TABLE IF NOT EXISTS `events` (
		...
		if(!mysqli_num_rows( mysqli_query($connection, "SHOW TABLES LIKE 'places'"))) {
			if(!mysqli_query($connection, "CREATE TABLE IF NOT EXISTS `places` (
			  `id` int(9) NOT NULL AUTO_INCREMENT,
		...
		if(!mysqli_num_rows( mysqli_query($connection, "SHOW TABLES LIKE 'settings'"))) {
			if(!mysqli_query($connection, "CREATE TABLE IF NOT EXISTS `settings` (
			  `sg_lastupdate` int(14) NOT NULL
			) ENGINE=MyISAM DEFAULT CHARSET=latin1;")) {
				die(mysql_error());
			}

			if(!mysqli_query($connection, "INSERT INTO `settings` (`sg_lastupdate`) VALUES (0)")) {
				die(mysql_error());
			}
		}
	}
