...
function getEventbriteEvents($eb_keywords, $eb_city, $eb_proximity) {
  ...
  foreach($xml->event as $event) {  
    ...
    $event_query = mysqli_query($connection, "SELECT * FROM events WHERE id_eventbrite=$event->id") or die(mysql_error());
    if(mysqli_num_rows($event_query) == 0) {
      ...
      mysqli_query($connection, "INSERT INTO events (id_eventbrite, 
                                      title,
                                      created, 
                                      organizer_name, 
                                      uri, 
