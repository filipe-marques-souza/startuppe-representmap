...
// get place info
$place_query = mysqli_query($connection, "SELECT * FROM places WHERE id='$place_id' LIMIT 1");
...
if($task == "doedit") {
  ...
  mysqli_query($connection, "UPDATE places SET title='$title', type='$type', address='$address', uri='$uri', lat='$lat', lng='$lng', description='$description', owner_name='$owner_name', owner_email='$owner_email' WHERE id='$place_id' LIMIT 1") or die(mysql_error());
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <select class="input input-xlarge" name="type">
          ...
          <option<? if($place['type'] == "aceleradora") {?> selected="selected"<? } ?>>aceleradora</option>
          <option<? if($place['type'] == "incubadora") {?> selected="selected"<? } ?>>incubadora</option>
          <option<? if($place['type'] == "coworking") {?> selected="selected"<? } ?>>coworking</option>
          <option<? if($place['type'] == "investidor") {?> selected="selected"<? } ?>>investidor</option>
