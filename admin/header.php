...
// get marker totals
$total_approved = mysqli_num_rows(mysqli_query($connection, "SELECT id FROM places WHERE approved='1'"));
$total_rejected = mysqli_num_rows(mysqli_query($connection, "SELECT id FROM places WHERE approved='0'"));
$total_pending = mysqli_num_rows(mysqli_query($connection, "SELECT id FROM places WHERE approved IS null"));
$total_all = mysqli_num_rows(mysqli_query($connection, "SELECT id FROM places"));

// admin header
$admin_head = "
  <html>
  <head>
    <title>Startup PE Admin</title>
