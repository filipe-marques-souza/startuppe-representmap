...
if($task == "hide") {
  ...
  mysqli_query($connection, "UPDATE places SET approved=0 WHERE id='$place_id'") or die(mysql_error());
...
if($task == "approve") {
  $place_id = htmlspecialchars($_GET['place_id']);
  mysqli_query($connection, "UPDATE places SET approved=1 WHERE id='$place_id'") or die(mysql_error());
  header("Location: index.php?view=$view&search=$search&p=$p");
  exit;
}
...
if($task == "delete") {
  $place_id = htmlspecialchars($_GET['place_id']);
  mysqli_query($connection, "DELETE FROM places WHERE id='$place_id'") or die(mysql_error());
  header("Location: index.php?view=$view&search=$search&p=$p");
  exit;
}
...
// get results
if($view == "approved") {
  $places = mysqli_query($connection, "SELECT * FROM places WHERE approved='1' ORDER BY title LIMIT $page_start, $items_per_page");
  $total = $total_approved;
} else if($view == "rejected") {
  $places = mysqli_query($connection, "SELECT * FROM places WHERE approved='0' ORDER BY title LIMIT $page_start, $items_per_page");
  $total = $total_rejected;
} else if($view == "pending") {
  $places = mysqli_query($connection, "SELECT * FROM places WHERE approved IS null ORDER BY id DESC LIMIT $page_start, $items_per_page");
  $total = $total_pending;
} else if($view == "") {
  $places = mysqli_query($connection, "SELECT * FROM places ORDER BY title LIMIT $page_start, $items_per_page");
  $total = $total_all;
}
if($search != "") {
  $places = mysqli_query($connection, "SELECT * FROM places WHERE title LIKE '%$search%' ORDER BY title LIMIT $page_start, $items_per_page");
  $total = mysqli_num_rows(mysqli_query($connection, "SELECT id FROM places WHERE title LIKE '%$search%'")); 
}
